                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-tools">
                                <a href="#" class="btn btn-primary btn-sm newAssetBtn">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" name="from_clinic_id" class="form-control"
                                            value="{{ $clinic->id }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="transferAssetTo">Transfer To</label>
                                        <select id="transferAssetTo" name="to_clinic_id" class="form-control select2"
                                            style="width: 100%;">
                                            <option selected="selected" disabled="disabled">
                                                Choose Clinic asset transfered to
                                            </option>
                                            @forelse ($org_clinics as $org_clinic)
                                                <option value="{{ $org_clinic->id }}">
                                                    {{ $org_clinic->clinic }}
                                                </option>
                                            @empty
                                                <option disabled="disabled">
                                                    No Any other clinic found!
                                                </option>
                                            @endforelse
                                        </select>
                                    </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="transferAssetDate" name="transfer_date" class="form-control datepicker" placeholder="Transfer Date">
                                    </div>
                                </div>
                                <!--.col-md-6-->
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function find_assets() {
                var path = '{{ route('admin.assets.index', $clinic->id) }}';
            ...
            $('#newAssetForm').submit(function(e) {
                ...
                var path = '{{ route('admin.assets.store') }}';
                $.ajax({
                    url: path,
                    type: 'POST',
                    data: formData,
            ...
            $(document).on('click', '.editItem', function(e) {
                ...
                var path = '{{ route('admin.assets.show') }}';
                $.ajax({
                    url: path,
                    type: 'POST',
                    data: {
                    ...
                    success: function(data) {
                        if (data['status']) {
                            ...
                            $('#updateAssetType').val(data['data']['type_id']);
                            $('#updateAssetCondition').val(data['data']['condition_id']);
            ...
            $('#updateAssetForm').submit(function(e) {
                ...
                var form = $(this);
                var formData = new FormData(form[0]);
                let path = '{{ route('admin.assets.update') }}';
                $.ajax({
                    url: path,
                    type: 'POST',
                    data: formData,
                    ...
                    complete: function() {
                        $('#updateAssetSubmitBtn').html('Save');
            ...
            $(document).on('click', '.deleteItem', function(e) {
                ...
                var asset_id = $(this).data('id');
                var token = '{{ csrf_token() }}';
                var path = '{{ route('admin.assets.delete') }}';
                Swal.fire({
                    title: "Are you sure?",
            ...
            $('#transferAssetForm').submit(function(e){
                ...
                var form = $(this);
                var formData = new FormData(form[0]);
                let path = '{{ route('admin.asset.tranfer.store') }}';
                $.ajax({
                    url: path,
                    type: 'POST',
                    data: formData,
